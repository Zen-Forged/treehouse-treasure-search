...
| Route | Purpose |
|---|---|
| `/` | Discovery feed — paper masonry grid (v1.1i), `<MallScopeHeader>` (session 68) above the `<FeaturedBanner>` eyebrow variant, mall selector via `<MallSheet>`, sticky masthead via `<StickyMasthead>`. |
...
| `/flagged` | **Find Map** — saved finds organized as a journal itinerary. `<MallScopeHeader>` above the overlay-variant `<FeaturedBanner>` (session 68). Pin retired — mall context now comes from the scope header, not save-pin extraction. X = booth glyph still anchors each spine stop (locked session 17). Spine connects to terminal closer circle (v1.1l). Tagline fragment as closer copy. Empty-state callout when active mall filter excludes all current saves. |
...
| `/shelf/[slug]` | Same as `/vendor/[slug]` — alternate URL scheme, same primitive. Session 67 added a masthead bookmark glyph (next to the share airplane), hidden when viewing your own booth. |
| `/shelves` | **Booths** — public booth directory across all malls. Open to every visitor (session 67). `<MallScopeHeader>` (session 68) above the grid; per-mall section headers when no mall picked, flat grid when one mall picked. Bookmarked-chip filter composes via intersection. Each non-admin tile carries a `<BookmarkBoothBubble>`. Admin entry is the masthead Admin pill, not a nav tab. |
...
- **`lib/auth.ts`** — `sendMagicLink(email, redirectTo?)` (appends `next=` for post-auth redirect), `getSession()`, `getUser()`, `signOut()`, `onAuthChange()`, `isAdmin(user)`, `detectUserRole(user)` (async — `'admin' | 'vendor' | 'shopper' | 'none'`, queries vendors + shoppers in parallel, drives post-sign-in routing + role-conditional chrome — added session 113), `getCachedUserId()`. Session-user-id cached to `localStorage.treehouse_auth_uid` for sync owner detection.
- **`lib/useShopperAuth.ts`** (session 111) — Shopper auth-state hook. Returns `{ isLoading, isAuthed, shopper }` reactive on `onAuthChange`. Crucially distinguishes "guest" (`isAuthed=false`) from "authed but pre-claim" (`isAuthed=true`, `shopper=null`) — a vendor user without a `shoppers` row reports the latter, so routing can send them through the claim flow rather than back to sign-in. Exports `initialsFromHandle(handle)` per design D11. Consumers: `(tabs)/layout.tsx` (masthead bubble swap), `/flagged` (sync footer gate), `/me` (auth-state branching).
...
- **`lib/safeStorage.ts`** — localStorage wrapper with sessionStorage + in-memory fallback. Use instead of raw `localStorage` in ecosystem client components (Safari ITP-safe). Known keys: `treehouse_auth_uid`, `treehouse_mode`, `treehouse_feed_scroll`, `treehouse_bookmark_<postId>`, `treehouse_last_viewed_post`, **`treehouse_active_vendor_id`** (session 35), **`treehouse_saved_mall_id`** (cross-tab mall filter, session 68 — see `useSavedMallId` hook), **`treehouse_booth_bookmark_<vendorId>`** (booth bookmarks, session 67).
- **`lib/useSavedMallId.ts`** (session 68; session 110 cross-instance sync fix) — Mall-filter hook. Returns `[savedMallId, setSavedMallId]`; reads `treehouse_saved_mall_id` via `safeStorage` on mount + listens to `treehouse:saved_mall_change` window event for in-tab cross-instance sync + `storage` event for cross-tab sync. The original D9 mount-time-read-only assumption broke when session 109's `app/(tabs)/layout.tsx` shared layout went in (the layout persists across tab switches, so its hook instance never re-mounted). Don't regress to the simpler shape. Setter dispatches the custom event so sibling instances stay in sync. Consumers: `(tabs)/layout.tsx` (postcard mall card + Saved badge), `(tabs)/page.tsx` (Home), `(tabs)/flagged/page.tsx` (Saved), `(tabs)/map/page.tsx` (Map).
- **`lib/useShopperSaves.ts`** (session 112) — Hybrid saves hook. Single source of truth for "which finds is this user saving?" Returns `{ ids, isLoading, isSaved, toggle }`. Three internal modes: guest → localStorage; authed + no shopper row → localStorage (pre-claim window); authed + shopper row → `shopper_saves` table. Cross-instance sync via `treehouse:saves_change` custom event with `{ id, next }` detail; idempotent via Set semantics so the toggling instance no-ops. Cross-tab sync via storage events (guest path only). Optimistic toggle with DB-error rollback that REBROADCASTS the rollback as a fresh event so sibling instances don't drift. Idempotent UPSERT via composite PK `(shopper_id, post_id)`. Consumers: `(tabs)/layout.tsx`, `(tabs)/page.tsx` (Home tile heart), `(tabs)/flagged/page.tsx`, `find/[id]/page.tsx` (save bubble), `my-shelf/page.tsx`, `shelves/page.tsx`, `shelf/[slug]/page.tsx`, `me/page.tsx`.
- **`lib/useShopperBoothBookmarks.ts`** (session 112) — Parallel hook for booth bookmarks. Same shape as useShopperSaves, keyed by vendor_id, writes to `shopper_booth_bookmarks`. Custom event `treehouse:booth_bookmarks_change` with `{ vendorId, next }`. Consumers: `shelves/page.tsx`, `shelf/[slug]/page.tsx`, `me/page.tsx`.
...
- **`<MallSheet>`** (v1.1i, session 21A) — bottom-sheet mall selector. Transform-free centering per session-21A fix. Consumers: Home feed (`/`), Booths (`/shelves`), Find Map (`/flagged`) — all three primary tabs. Session 68 added optional `countUnit?: { singular, plural }` prop (defaults to "find" / "finds") so the per-mall count column reads "4 booths" on Booths and "2 saved finds" on Find Map. Pending migration to `/post`, `/post/preview`, `/vendor-request` (deferred as v1.1k (h)).
- **`<MallScopeHeader>`** (session 68) — three-callsite mall-scope chrome (Home / Booths / Find Map). Italic eyebrow + tappable bold name + chevron-down + optional geo line (`address` kind with map link, or `italic` kind for context text). Content-agnostic — consumer supplies eyebrow strings ("Finds from", "Booths at", "Saves at"). Tapping the name opens `<MallSheet>` via parent's `onTap` callback. Persistence + sheet state live in the parent. Per session-68 iPhone QA, this primitive sits ABOVE any hero/banner on its host page (state controls > page-title chrome — see memory `feedback_state_controls_above_hero_chrome.md`).
