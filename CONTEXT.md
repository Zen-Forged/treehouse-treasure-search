...
- **`lib/useSavedMallId.ts`** (session 68) — Cross-tab mall-filter hook. Returns `[savedMallId, setSavedMallId]`; reads `treehouse_saved_mall_id` via `safeStorage` once on mount, writes through on update. Mount-time read only — bottom-nav tab switches in App Router unmount/remount the page, so no `storage`-event subscription or in-app event bus needed. Three consumers: Home (`/`), Booths (`/shelves`), Find Map (`/flagged`). Pick a mall on any of the three; the other two honor it on next visit.
