...
## ✅ Session 93 (2026-04-30) — Login triage shipped end-to-end + iPhone QA passed — 1 commit + close

David's session 92 closer recommended iPhone QA of Wave 1 follow-up + R1 design pass. He redirected at standup to a focused login triage cleanup — vendors hitting `/login` had no callout for whether to sign in or create an account, and the magic-link path would still authenticate brand-new users without a vendor record (orphan `auth.users` row, lands on /my-shelf empty-state with no path back). One arc, one commit, iPhone QA passed.

**Arc — login triage scoping → mockup → ship.** Three-phase shape:

| Phase | Output | Key moves |
|---|---|---|
| Scoping | Read current `/login`, audit inbound `/login` callsites, surface concerns | Pre-emptively read /login + grep all callsites BEFORE drafting (per session-92 lesson on auth/identity conflation discovery). Surfaced 11 concerns ranked by leverage: 🔴 Google OAuth is new capability (defer V1), 🔴 R1 framing breaks "Vendor Sign In" later (vendor-only V1, retitle on R1), 🟡 orphan-auth-user persists (accept for V1), 🟡 BottomNav Profile-tap destination (recommend `/login`=triage, `/login/email`=form), 🟡 magic-link callback routing (path a — keep callback at /login, route via `?confirmed=1` branch). David approved scope with "Continue with Google" deferred + footer illustration dropped + wordmark from existing `public/wordmark.png`. |
| Mockup V1 | `docs/mockups/login-triage-v1.html` — 4 frames | Frame 1+2 = Screen 1 hero treatment options (wordmark + welcome text vs wordmark only). Frame 3 = Screen 2 form refined. Frame 4 = authed-user state alternative. David picked: Q1=(b) wordmark only, Q2=(a) skip triage for authed users (current behavior), Q3=(a) keep "Email me a code" copy. Q4 redirects: vertical centering + warmer white on cards/inputs + flagged the guest-account question. |
| Mockup V2 | `docs/mockups/login-triage-v2.html` — 3 frames (fill-refinements within picked Frame 2) | Frame 1 = vendor-only (as scoped). Frame 2 = + "Just shopping? Keep browsing — no account needed." disambiguation line. Frame 3 = Screen 2 with V2 styling applied. David picked Q1=(b) guest line included. **3rd firing of `feedback_v2_mockup_as_fill_refinement.md`** — V2 was fill-refinements within Frame 2 (the V1 pick), not a structural reset. |

**Implementation (1 commit, `26f0b66`).** Two routes from one:

| Route | What |
|---|---|
| `/login` | New triage page. Renders Frame 2 picked: wordmark + subtitle + 2 cards (Sign in filled / Create Vendor outlined) + "Just shopping?" line + "Need help? Contact us" footer. State machine: `confirming` (`?confirmed=1` magic-link bridge polling), `loading` (auth-state hold to avoid flashing cards at authed user mid-redirect), `triage` (cards). Forwarding logic: explicit `?redirect=` → `/login/email?redirect=...`, authed user → `/login/email`. Magic-link callback URL preserved at `/login` per path (a) decision — no `lib/auth.ts` or email-template changes needed. |
| `/login/email` | The OTP form, refined. Vertically centered, `v1.postit` (#fbf3df) on email input + bottom "Create a vendor account" callout, sign-out italic dotted-link below the form (visible when authed). |

**Two inbound callsites upgraded to skip triage** (known returning users): `/vendor-request` "Sign in instead" CTA + `/my-shelf` re-auth bounce both go directly to `/login/email` rather than through triage. Other bare-`/login` callsites (`/admin/login`, `/setup` `handleGoToLogin`) intentionally left going to triage — those are user-initiated "go to login chooser" not "re-auth a known returning user."

**Two self-caught bugs fixed before push:**
1. **React hook-ordering bug** — initial implementation called `useState`/`useEffect` after an early-return for `confirming` mode. Caught on re-read; restructured to single state machine with all hooks above any conditional render.
2. **`URLSearchParams` typing** — `pickDest` parameter typed as `URLSearchParams` failed against `useSearchParams()`'s `ReadonlyURLSearchParams`. One-line fix.

**iPhone QA: PASSED end-to-end** (David walked the routing list across 6+ paths — bare guest, "Sign in" tap → email form, "Create Vendor" tap, "Need help?" tap, magic-link callback, and Profile-tap-when-authed forwarding).

**Memory updates:** None this session.
- 3rd firing of `feedback_v2_mockup_as_fill_refinement.md` (already memorialized session 82).
- `feedback_mid_session_iphone_qa_on_vercel_preview.md` applied — David QA'd on Vercel preview rather than waiting for next session.
- 3 single-firing patterns from this session (triage-first routing for ambiguous-state pages / inbound deep-link audit when splitting routes / forward-known-states + render-fresh-states routing primitive) — Tech Rule queue candidates on second firings; not memorialized.

**Operational follow-ups (carry into session 94):**

- **🟡 NEW PRIMARY (carry from session 92): iPhone QA of Wave 1 follow-up commits.** Three design refinements (`8ec7884` contact email split, `4b515b2` mall hero relocation, `b6919b7` EditBoothSheet hero consolidation) landed in code session 92 but were not device-walked. Specific watch-items per session 92 close still apply.
- **🟢 NEW (session 93): R1 (shopper accounts) routing slot pre-baked.** When R1 ships: (a) Screen 2 retitle "Vendor Sign in" → "Sign in", (b) Screen 1 third card "I'm shopping" added, (c) "Just shopping?" line retires (replaced by the third card), (d) subtitle generalizes. All four changes contained to `app/login/page.tsx` + `app/login/email/page.tsx` — no broader impact.
- **🟢 NEW (session 93): Other inbound `/login` callsites left routing through triage.** `/admin/login` "go to vendor login" + `/setup` `handleGoToLogin` both bare-`/login`. If user testing reveals friction (extra tap to reach the form), single-line update each.
- 🟡 CARRY (session 92): `inspect-grants.ts` heuristic refinement (~20 min).
- 🟡 CARRY (session 92): Migration 010 + 011 staging gap — `events` table missing on staging.
- 🟢 CARRY (session 92): AI-route auth deferred.
- 🟡 CARRY (session 91): `vendor_request_approved` admin actor lacking `_by_admin` suffix — cosmetic, defer to Q-014.
- 🟡 CARRY (session 91): Q-014 Metabase scoping — multiple session-related event-volume notes accumulating.
- 🟡 CARRY (session 91): Q-015 admin Vendors tab.
- 🟡 CARRY (session 91): feed pagination — Wave 1 explicitly out of scope.
- 🟡 CARRY (session 89): BoothHero pencil-vs-bookmark symmetry question. UNTOUCHED.
- 🟡 CARRY (session 88): motion design revisit in a focused full session. UNTOUCHED.
- 🟡 CARRY (session 88): preview-cache role audit. UNTOUCHED.
- 🟡 CARRY (session 87): wordmark asset-weight optimization. UNTOUCHED.
- 🟡 CARRY (session 87): docs/design-system.md wordmark spec stale. UNTOUCHED.

**Roadmap movement:** No new R-items. R1 prerequisites get sharper — login routing model now anticipates the 3rd shopper card slot.

**Commits this session (1 runtime + close):**

| Commit | Message |
|---|---|
| `26f0b66` | feat(login): split /login into triage + email form |
| (session close) | docs: session 93 close — login triage shipped + Wave 1 follow-up QA still carry |
...
> Older tombstones live in [`docs/session-archive.md`](docs/session-archive.md). Sessions 28–43 still awaiting archive-drift backfill (one-liner only); sessions 44–53 + 56 in archive (54 + 55 + 57–92 still missing — operational backlog growing). Session 87 mini-block rotated off the visible session-block area at session 93 close (one-liner stays here). Session 88 mini-block also retired at session 93 close (tombstone bullet below stays as historical record). Visible window now sessions 88–92.
...
_(Session 87 tombstone rotated off at session 93 close — was Tech Rule queue update reconciled (17 → 33 candidates registered, 11 🟢 promotion-ready) + brand asset overhaul end-to-end: full Treehouse Finds favicon + PWA icon set replacing Vercel default-N, masthead wordmark swap from session-80 TNR text to transparent-PNG logo system-wide. NEW memory `feedback_tech_rule_promotion_destination.md`. Smallest→largest 10th clean firing. 3 runtime commits.)_

---

## CURRENT ISSUE
> Last updated: 2026-04-30 (session 93 close — login triage shipped end-to-end. `/login` is now a triage page (returning vendor / new vendor cards + "Just shopping?" guest disambiguation + Contact us footer). `/login/email` is the OTP form, vertically centered, warm postit white on inputs. 1 runtime commit + close. iPhone QA passed end-to-end across 6+ routing paths. R1 shopper accounts routing slot pre-baked — when R1 ships, Screen 2 retitle + 3rd "I'm shopping" card + "Just shopping?" line retires.)

**Working tree:** clean (after close commit). **Build:** green. **Beta gate:** unblocked. **Net change this session:** 1 runtime commit + close. Login triage shipped. R1 hard prereqs all green from session 92; routing slot for R1's third shopper-account card now pre-baked. No regressions. Roadmap unchanged at 8/15.

### 🚧 Recommended next session — iPhone QA of Wave 1 follow-up + R1 (shopper accounts) design pass (~60–90 min)

Same shape as the session-92 closer's recommended next move — David picked a different direction this session (login triage), so the recommendation rolls forward. Wave 1 follow-up iPhone QA (~10 min) confirms the 3 design refinements landed cleanly. R1 design pass (~60–80 min, mockup-first) opens the biggest single swing on the remaining roadmap.

**Why this shape (over the alternatives):**
- 3 visible-state changes shipped session 92 that David hasn't device-walked yet (contact mailto, mall hero composition, EditBoothSheet hero section). Quick to verify; cheap if regression surfaces.
- R1 (shopper accounts) is the largest unblocked R-item. Wave 1.5 prereqs all green; login routing model from session 93 anticipates the 3rd shopper card slot — about 30% of R1's design work is already done. R1 unblocks R8 (onboarding) + R9 (push) + R14 (vendor enrichment) downstream.
...
3. **R1 design pass (~60–80 min, mockup-first).** Reseller-intel layer is `localStorage`-only by design; shopper accounts add a third tier (alongside vendor + admin) to the ecosystem layer. Login routing slot from session 93 already provides Screen 1 (triage cards) + Screen 2 (form); R1's job is to (a) add the 3rd "I'm shopping" card, (b) generalize Screen 2 title "Vendor Sign in" → "Sign in", (c) retire the "Just shopping?" line, (d) generalize subtitle. Other open questions to surface in mockup: (e) sign-up flow vs. magic-link only? (f) what does "saved finds" persistence look like for authed shoppers vs. guest-localStorage carry-forward? (g) Profile tab content for authed shoppers? (h) does shopper account invalidate the localStorage bookmark store on first auth? Mockup phone-frame HTML in `docs/mockups/r1-shopper-accounts-v1.html`. Plain-English decisions pane at top + 2-3 multiple-choice questions at bottom.
...
### Session 94 opener (pre-filled — iPhone QA of Wave 1 follow-up + R1 shopper accounts design pass)
...
CURRENT ISSUE: iPhone QA of session-92 follow-up + R1 shopper accounts design pass (~60–90 min). (1) iPhone QA across 3 design refinements: /contact mailto handoff, mall hero now BELOW MallScopeHeader at FeaturedBanner dimensions, /my-shelf BoothHero clean + EditBoothSheet hero photo section, /shelves admin EditBoothSheet has hero controls. (2) Address concrete issues. (3) R1 mockup-first design pass — login routing model from session 93 already anticipates the 3rd shopper-account card slot.

PROBLEMS SESSION 93 ALREADY SOLVED — don't accidentally revive: (a) /login is the triage page (returning vendor / new vendor cards + "Just shopping?" line + Contact us footer). DO NOT put the OTP form there. (b) /login/email is the OTP form (warm postit white inputs, "Create a vendor account" bottom callout, sign-out italic link below). (c) Magic-link callback URL stays at /login per path (a) decision — /login detects ?confirmed=1 and renders the polling bridge. DO NOT change emailRedirectTo in lib/auth.ts. (d) /vendor-request "Sign in instead" + /my-shelf re-auth bounce both route directly to /login/email (skip triage for known returning users). DO NOT revert to bare /login. (e) Authed users hitting /login forward to /login/email. (f) Frame 2 (wordmark only, no welcome text) is the picked Screen 1 hero treatment.
