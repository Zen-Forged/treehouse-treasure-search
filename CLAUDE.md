...
- **Tech Rule promotion batch — badly overdue, now richer with sessions 85+86 candidates.** 30+ candidates queued. **Top-of-list (full validation):** "Cap speculative patching at 3 rounds + escalate to device-level visibility" (2nd firing session 86 with full closure). **Other 🟢 promotion-ready:** TR-l + TR-m, "verify sub-agent claims that contradict known semantic rules", "smallest→largest commit sequencing" (9 firings: sessions 70 1st + 77 2nd misfire + 78 1st clean + 80 2nd clean + 81 3rd + 82 6th + 83 7th + 84 8th + 85 9th — load-bearing); `feedback_mockup_options_span_structural_axes.md` (4 firings); "Mid-session iPhone QA on Vercel preview" (3 firings); "Lora lineHeight 1.4 minimum" (2 firings); `feedback_recurring_phrase_signals_system.md` (2 firings); `feedback_visibility_tools_first.md` (3 firings, session 86 full closure case); `feedback_kill_bug_class_after_3_patches.md` (2nd clean firing session 86). **Session-86 single-firings:** (1) "Hypothesis + diagnostic + fix in same commit cycle when confidence ≥80%"; (2) "Refuse-to-write-0 pattern (write-side filter on meaningless restore values)". Full register: [`docs/tech-rules-queue.md`](docs/tech-rules-queue.md).
