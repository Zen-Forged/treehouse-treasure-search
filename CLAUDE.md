...
## ✅ Session 86 (2026-04-29) — /my-shelf admin scroll-restore CLOSED in 1 fix commit after Safari Web Inspector via USB connected; Phase 3 fully shipped across all 3 surfaces; 5-session carry-forward retired (3 runtime commits + close, net -63 lines after diagnostic retirement)

The session 85 close recommendation executed exactly as planned: connect Safari Web Inspector via USB cable as Part A, then close /my-shelf admin scroll-restore as Part B. **Total time from Inspector connected to bug shipped: ~30 minutes.** Compare session 85's 6 rounds of speculative patches without Inspector that landed nothing. **Full validation of the meta-rule "cap speculative patching at 3 rounds + escalate to device-level visibility"** captured at session 85 close.

**Arc 1 — Web Inspector via USB connected (Part A, ~10 min including HITL setup).** David walked through the 5-step setup procedure from [`reference_ios_safari_web_inspector.md`](memory): iPhone Settings → Safari → Advanced → Web Inspector ON; Mac Safari → Settings → Advanced → "Show features for web developers" ON; USB cable + trust prompt; Mac Safari → Develop menu → [iPhone name] → tab. First setup hit two snags worth capturing: (1) the iPhone admin session had logged out, so the first /my-shelf navigation routed to /shelf/<slug> via the non-admin branch in [app/shelves/page.tsx:117-118](app/shelves/page.tsx); David re-signed in as admin and /my-shelf?vendor=<id> engaged. (2) The iPhone wasn't appearing in Mac Safari's Develop menu initially — diagnostic checklist (Web Inspector toggle, cable type, trust prompt, Safari restart) resolved it in one round. Both snags now captured in the memory file as carry-forward troubleshooting notes.

**Arc 2 — Storage panel revealed root cause in seconds (Part B start).** With Inspector connected to `/my-shelf?vendor=<id>&debug=1` on iPhone, David opened the Storage tab. Both Local Storage AND Session Storage showed `treehouse_my_shelf_scroll = 0`. **The key was present, the value was 0** — meaning a handler was firing with `window.scrollY === 0` and clobbering the user's good scroll position. Session 85's diagnostic correctly fingered "saved is empty" because [readScrollY()](app/my-shelf/page.tsx) explicitly rejects values ≤ 0 (`y > 0` filter), so the read returned null and the diagnostic pill said `saved: —`. But the storage layer below React showed the *actual* issue: writes WERE landing, just with the wrong value.

**Arc 3 — Hypothesis + diagnostic + fix shipped same commit cycle (3 runtime commits).** Hypothesis: Next.js App Router scrolls to top on outbound navigation (default behavior); the scroll listener (still attached pre-unmount) catches those scroll events as `window.scrollY` animates from 300 → 0 and writes 0 to storage in 5-8 bursts per cycle. The bursts (visible as "(5)" and "(8)" badges in iPhone Console — Console's auto-collapse counter) confirmed the scroll-event-during-transition theory before any fix shipped.

- `788e5b5` Diagnostic: log stack trace whenever `writeScrollY` is called with `y === 0`. Pure visibility — no logic change. Console confirmed bursts of zero-writes every cycle.
- `0c53fb3` Fix: refuse to write 0 in `writeScrollY` (write-side filter). Each listener also threaded its event type through to writeScrollY so any remaining zero-write paths log the responsible event in plain English (not stack traces). David's iPhone QA confirmed: scroll restore lands each time. **Bug closed.**
- `e311dce` Cleanup: retire the diagnostic infrastructure that was always meant to come out post-fix. Removed the `diag` state object + 12 `setDiag` call sites + the diagnostic pill JSX + the `console.warn` in `writeScrollY` + per-listener event-name tracking. Net **-85 lines** from the file.

**Arc 4 — Pattern proof on the meta-rule.** Session 85 spent 6 rounds patching speculatively at progressively deeper layers ABOVE the actual bug (BFCache → router cache → dual-write storage → popstate → useSearchParams reactivity). None of those layers was wrong as defensive code, but none was the layer where the bug lived. **The bug lived in the *write side*: the scroll listener was catching real scroll events generated by Next.js's outbound-navigation scroll-to-top.** That's invisible to React state, invisible to on-screen diagnostics, only visible to a tool that can see *into* browser/storage lifecycle — Safari Web Inspector. Once Inspector was connected, the root cause was visible in the Storage panel within seconds. **The fix was 1 line of logic** (`if (rounded <= 0) return`), which retired -85 lines of session-85-era speculation — exactly what `feedback_kill_bug_class_after_3_patches.md` predicts: structural fix < accumulated patches. The session 85 structural patches (BFCache pageshow handler, popstate, useSearchParams reactivity, dual-write storage, retry-scrollTo loop) all stay in place — harmless and may be load-bearing on devices/browsers we haven't QA'd against. Aggressive revert to /flagged-style minimum viable code is a follow-up under extended QA, not this session.

**Final state in production (as of `e311dce` runtime + close):**

- ✅ /my-shelf admin scroll-restore working (confirmed iPhone QA in-session, multiple cycles).
- ✅ Phase 3 scroll-restore now closed across **all three** target surfaces (/flagged, /shelf/[slug] guest path, /my-shelf admin path). 5-session carry-forward retired.
- ✅ Safari Web Inspector via USB cable now an operational primitive — David has it set up; future iOS-Safari bugs tractable in minutes via this path.
- 🟢 No regressions to Track D shared-element transitions, /transition-test testbed, or the previously-shipped Phase 3 surfaces.
- 🟡 /find/[id] peer-nav flag glyph + horizontal carousel scroll loss — out-of-scope from session 85, real bug, separable session. Now Inspector-debuggable the same way.

**Commits this session (3 runtime + close):**

| Commit | Message |
|---|---|
| `788e5b5` | diag(/my-shelf): log stack trace when writeScrollY writes 0 |
| `0c53fb3` | fix(/my-shelf): refuse zero-writes in scroll-restore + identify event |
| `e311dce` | chore(/my-shelf): retire scroll-restore diagnostic infrastructure |
| (session close) | docs: session 86 close — /my-shelf scroll-restore CLOSED + Web Inspector validated as institutional primitive |

**What's broken / carried (delta from session 85):**

- 🟢 RESOLVED: **/my-shelf admin scroll-restore** (sessions 79 + 85 attempts). Closed with Inspector + 1 fix commit. Phase 3 now fully shipped across all 3 surfaces.
- 🟢 RESOLVED: **Safari Web Inspector via USB cable setup** (carry from session 85). David has it working end-to-end; memory file `reference_ios_safari_web_inspector.md` updated with session 86 outcome + the two setup-time snags hit.
- 🟡 CARRY: /find/[id] peer-nav flag glyph + horizontal carousel scroll loss (from session 85). Still ~30 min standalone session. Now Inspector-debuggable.
- 🟡 CARRY: Session 85's structural patches on /my-shelf (BFCache pageshow handler, popstate, useSearchParams reactivity, dual-write storage, retry-scrollTo loop). Harmless, possibly vestigial. Aggressive cleanup deferred.
- 🟡 All session 78–85 carry-forwards still hold unless explicitly resolved above.

**Memory updates:**

- Updated [`feedback_visibility_tools_first.md`](/Users/davidbutler/.claude/projects/-Users-davidbutler-Projects-treehouse-treasure-search/memory/feedback_visibility_tools_first.md) — **3rd firing total** (R3 events / scroll-restore React layer / scroll-restore browser layer). Promotion-ready as Tech Rule. Session 86 addendum captures the full closing arc: Inspector connected → Storage panel showed actual value → 1 diagnostic + 1 fix + 1 cleanup → ~30min total.
- Updated [`reference_ios_safari_web_inspector.md`](/Users/davidbutler/.claude/projects/-Users-davidbutler-Projects-treehouse-treasure-search/memory/reference_ios_safari_web_inspector.md) — session 86 outcome section: setup procedure validated; 2 setup snags captured; the lesson generalized as "Inspector first, not last, for iOS-Safari bugs past round 2."
...
- `feedback_kill_bug_class_after_3_patches.md` — full validation. The rule predicted "structural fix is usually less code" — fix was 1 line of logic, which retired 85 lines of session-85 speculation. **Promotion-ready on this 2nd clean firing.**
- `feedback_visibility_tools_first.md` — 3rd firing, promotion-ready.
- `reference_ios_safari_web_inspector.md` — primary instruction set, validated end-to-end this session.

**Live discoveries (single firings, Tech Rule candidates):**

- **"Hypothesis + diagnostic + fix in the same commit cycle when confidence ≥80%"** — first firing as structural pattern. After Inspector showed bursts of 5-8 zero-writes per cycle, confidence was 90% on "Next.js scroll-to-top + scroll listener." Shipped diagnostic AND fix in `0c53fb3` rather than waiting for another verify-then-fix round. Compresses 2 cycles into 1.
- **"Refuse-to-write-0 pattern (write-side filter on meaningless restore values)"** — first firing as a project-specific scroll-restore primitive. If any future scroll/persistence primitive needs the same defense, the canonical fix shape is to filter at the write site, not the read site. Captured in code comments at [app/my-shelf/page.tsx](app/my-shelf/page.tsx); not a memory file.

**Operational follow-ups:**

- **NEW PRIMARY: Tech Rule promotion batch — now severely overdue + with 3 brand-new promotion-ready candidates** from sessions 85+86. ~30 min ops pass into [`docs/tech-rules-queue.md`](docs/tech-rules-queue.md): smallest→largest sequencing (9 firings, load-bearing across every David-redirect-bundle); structural-mockup-axes (4 firings); mid-session-iPhone-QA (3 firings); Lora-lineHeight-1.4-minimum (2 firings); recurring-phrase-signals-system (2 firings); + 4 session-84 single-firings; + 3 session-85 single-firings; + 2 session-86 single-firings; **plus the cap-speculative-patching-at-3-rounds rule, now 2nd firing with full validation — top-of-list landing**.
- **NEW CARRY: /find/[id] peer-nav fix** — flag glyph + horizontal carousel preservation; ~30 min standalone, now Inspector-debuggable.
- **All session 78–85 carry-forwards still hold** unless explicitly resolved above.
...
- Archive-drift backfill: tombstones for sessions 54+55+57–85 still missing from `docs/session-archive.md`. Ops candidate.

**Notable artifacts shipped this session:**

- [`app/my-shelf/page.tsx`](app/my-shelf/page.tsx) — refused-zero-writes fix shipped + diagnostic infrastructure retired (-85 lines net).
- Memory: extended `feedback_visibility_tools_first.md` (3rd firing, promotion-ready) + `reference_ios_safari_web_inspector.md` (outcome + setup-snag carry-forwards).
...
> Older tombstones live in [`docs/session-archive.md`](docs/session-archive.md). Sessions 28–43 still awaiting archive-drift backfill (one-liner only); sessions 44–53 + 56 in archive (54 + 55 + 57–85 still missing — operational backlog growing). Session 78 fell off the bottom of last-5 visible tombstones at session 84 close; session 80 mini-block rotated off the visible list at session 85 close; session 81 mini-block rotated off the visible list at session 86 close (still in this tombstones list as one-line entry).
...
_(Session 79 + 78 mini-blocks rotated off visible tombstones list at session 85 close. Session 80 mini-block rotated off the visible tombstones list at session 86 close — full block was already consolidated into a one-liner above. Session 81 mini-block also rotated off at session 86 close — net visible window now sessions 82–86. Session 79 was the original Track D + scroll-restore attempt that fully reverted in `a9dc1bd`; session 85 successfully re-attempted the scroll-restore portion on 2-of-3 surfaces; session 86 closed the third. Session 78 was the original Track D shared-element ship across feed → /find/[id] + /flagged → /find/[id] + /shelves → /shelf/[slug] surfaces. Sessions 80 + 81 had design-bundle work — see commit history for details.)_
---

## CURRENT ISSUE
> Last updated: 2026-04-29 (session 86 close — /my-shelf admin scroll-restore CLOSED in 1 fix commit + 1 cleanup after Safari Web Inspector via USB connected. Phase 3 scroll-restore now fully shipped across all 3 surfaces. The 5-session carry-forward retired. Full validation of session 85's "cap speculative patching at 3 rounds + escalate to device-level visibility" meta-rule — Inspector connected → root cause visible in Storage panel within seconds → fix shipped in ~30 minutes total. 3 runtime commits + close, net -63 lines after diagnostic retirement.)

**Working tree:** clean. **Build:** green. **Beta gate:** technically unblocked since session 50; no code-level regressions. **Net change this session:** 3 runtime commits + close — Phase 3 scroll-restore primitive now closed end-to-end (`/flagged` + `/shelf/[slug]` guest path session 85; `/my-shelf` admin path session 86). Bug was Next.js App Router scroll-to-top firing real scroll events on outbound navigation; the scroll listener wrote 0 to storage, clobbering the user's good scroll position. One-line write-side fix (`if (rounded <= 0) return`). **Safari Web Inspector via USB cable is now an operational primitive** — first iOS-Safari bug closed using this path; future ones tractable in minutes via the same procedure.

### 🚧 Recommended next session — Tech Rule promotion batch (~30 min ops pass)

Severely overdue and now richer than ever — sessions 85+86 added 3 brand-new promotion-ready candidates plus full validation on multiple existing ones.

**Why now (over the alternatives):**
- The biggest carry-forward (5-session /my-shelf scroll-restore) just retired — natural moment to consolidate.
- Two top-of-list landings now have full validation: (a) "cap speculative patching at 3 rounds + escalate to device-level visibility" (1st firing session 85, 2nd firing session 86 closed the bug exactly as the rule predicted); (b) "structural fix < accumulated patches" via `feedback_kill_bug_class_after_3_patches.md` (1 line of fix retired -85 lines of session-85 speculation). Both rules are now load-bearing, not theoretical.
- Pure ops session — no production code changes, no QA risk. Lands the institutional learning before context degrades.
- Smallest→largest commit sequencing now **9 firings** load-bearing across every David-redirect-bundle workflow. Promotion-overdue.

**Items to land in [`docs/tech-rules-queue.md`](docs/tech-rules-queue.md):**

1. **🟢 Cap speculative patching at 3 rounds + escalate to device-level visibility** — 2nd firing with full session-86 validation. Top-of-list.
2. **🟢 Smallest→largest commit sequencing** — 9 firings, load-bearing. Severely overdue.
3. **🟢 Structural fix < accumulated patches** — `feedback_kill_bug_class_after_3_patches.md` 2nd clean firing. Promotion-ready.
4. **🟢 Visibility tools first** — `feedback_visibility_tools_first.md` 3rd firing total. Promotion-ready.
5. **🟢 Mockup options span structural axes** — 4 firings. Promotion-ready.
6. **🟢 Mid-session iPhone QA on Vercel preview** — 3 firings. Promotion-ready.
7. **🟢 Lora needs lineHeight 1.4 minimum** — 2 firings (vendor names + tile titles). Promotion-ready.
8. **🟢 Recurring-phrase-signals-system** — 2 firings. Promotion-ready.
9. **+ 4 session-84 single-firings** (security audit work).
10. **+ 3 session-85 single-firings** (visibility-tool layer matching, cap-speculation rule, storage-layer-below-React).
11. **+ 2 session-86 single-firings** (hypothesis-fix-same-commit-cycle, refuse-to-write-0 pattern).
...
2. **Audit gap follow-ups (security continuation).** Runbook from session 84 flagged 5 unbuilt categories: function `search_path` mutability, role-grant drift, `auth.users` exposure, OTP/password policy, per-route `/api/*` auth audit. ~30–60 min standalone or ~2 hrs bundled. Strongly justified now that real vendor data is flowing. Infrastructure in place (audit scripts under `scripts/security-audit/`); each follow-up is a probe + migration + runbook update.
3. **Aggressive cleanup of /my-shelf scroll-restore code** (~20 min). Session 85's structural patches (BFCache pageshow handler, popstate, useSearchParams reactivity, dual-write storage, retry-scrollTo loop) all stay in place at session 86 close. They're harmless and possibly load-bearing on devices we haven't QA'd against — but with the actual root cause now identified and fixed, they may all be vestigial. Revert to /flagged-style minimum viable code, ship, run iPhone QA across the 3 Phase 3 surfaces. If clean, retain the simpler version.

Full alternatives + operational backlog in [`docs/queued-sessions.md`](docs/queued-sessions.md).

### Session 87 opener (pre-filled — Tech Rule promotion batch)
...
CURRENT ISSUE: Tech Rule promotion batch — severely overdue ops pass (~30 min). Land into [`docs/tech-rules-queue.md`](docs/tech-rules-queue.md): (1) cap-speculative-patching-at-3-rounds + escalate-to-device-level-visibility (2nd firing with session-86 full validation — top-of-list); (2) smallest→largest commit sequencing (9 firings, load-bearing); (3) structural fix < accumulated patches via feedback_kill_bug_class_after_3_patches.md (2nd clean firing); (4) visibility tools first via feedback_visibility_tools_first.md (3rd firing total); (5) mockup options span structural axes (4 firings); (6) mid-session iPhone QA (3 firings); (7) Lora lineHeight 1.4 minimum (2 firings); (8) recurring-phrase-signals-system (2 firings); + 4 session-84 single-firings (security audit work) + 3 session-85 single-firings (visibility-tool-layer-matching, cap-speculation, storage-layer-below-React) + 2 session-86 single-firings (hypothesis-fix-same-commit-cycle, refuse-to-write-0 pattern). ALTERNATIVES IF DEFERRED: (1) /find/[id] peer-nav fix — flag glyph + horizontal carousel preservation when navigating /find/[id-A] → "More from this booth" → /find/[id-B] → back. ~30 min standalone, now Inspector-debuggable. (2) Audit gap follow-ups — function search_path, role grants, auth.users exposure, OTP/password policy, per-route /api/* auth. (3) Aggressive cleanup of /my-shelf scroll-restore code — revert session-85 structural patches now that root cause is fixed at the write site. CARRY-FORWARDS FROM SESSIONS 78–86: Phase 3 scroll-restore CLOSED end-to-end across /flagged + /shelf/[slug] guest + /my-shelf admin (sessions 85+86); preview cache pattern (sessionStorage `treehouse_find_preview:${id}`) reusable for shared-element work; framer-motion layoutId is a global namespace; testbed `/transition-test` should be USED for surface-extension work; Track D phase 5 source layoutId retired on /shelves session 80; wordmark on every masthead is TNR upright ink-black 18px (FONT_NUMERAL); /shelves is row pattern not grid; bookmark on BoothHero photo corner not in masthead; IM Fell COMPLETELY RETIRED (FONT_LORA token); BoothLockupCard shared primitive owns /shelves + /flagged; Polaroid evolved tile pattern owns Home + /flagged + /shelf/[slug] WindowTile + ShelfTile; Title+price on tiles is centered Lora 14px group, height locked 76 / 56; /find/[id] strip got centering WITHOUT polaroid per browse-vs-navigate rule; RLS enabled on every public table (migrations 014+015), audit_rls_state() RPC live; security-audit infrastructure under scripts/security-audit/ + runbook. From session 86: refuse-to-write-0 is the canonical scroll-restore primitive — write-side filter on meaningless restore values; Safari Web Inspector via USB cable is the operational primitive for iOS-Safari bugs past round 2; /find/[id] peer-nav flag-glyph + horizontal carousel scroll loss is still the active follow-up.
```
...
- **Tech Rule promotion batch — queue landed session 87 (33 candidates: 11 🟢 promotion-ready, 22 🟡 at first firing).** Session 87 was the queue-update ops pass: bumped TR-m to 3× firings (sessions 58 + 59 + 86 closure); added TR-r through TR-z (9 new 🟢 promotion-ready rows: cap-speculative-patching-at-3-rounds, smallest→largest commit sequencing, structural-fix-less-code, mockup-options-span-structural-axes, mid-session-iPhone-QA, Lora-lineHeight-1.4, recurring-phrase-signals-system, user-facing-copy-scrub, V2-mockup-as-fill-refinement); added TR-aa through TR-ag (7 new 🟡 single-firings covering session-84 security-audit operating principles + session-85 storage-layer-below-React + the two session-86 patterns). **Next phase — promotion-out-of-queue batch:** write prose entries into [`docs/DECISION_GATE.md`](docs/DECISION_GATE.md) `## The Tech Rules` block for the 11 🟢 candidates. Top-of-list landing: TR-r (cap-speculative-patching), TR-s (smallest→largest, severely overdue at 9 firings), TR-t (structural-fix-less-code, paired with TR-r). Full register: [`docs/tech-rules-queue.md`](docs/tech-rules-queue.md).
