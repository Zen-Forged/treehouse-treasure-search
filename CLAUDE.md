...
## ✅ Session 84 (2026-04-29) — Security audit infrastructure shipped + `rls_disabled_in_public` exposure closed end-to-end across both Supabase projects (3 runtime commits + new diagnostic suite + runbook)

A pure-security session triggered by Supabase Security Advisor emails flagging `rls_disabled_in_public` on `site_settings` + `events` for both prod (`zogxkarpwlaqmamfzceb`) and the staging project. **Smallest→largest commit sequencing's 8th clean firing — promotion-overdue.** **Script-first over SQL-paste-first** (MASTER_PROMPT.md §Data/schema fixes) was load-bearing throughout — every Supabase investigation produced a committed diagnostic, not a chat-only one.

**Diagnosis arc.** Read migrations 002 + 003 + 008 + 010 + 011 to map the existing RLS posture. Found that `site_settings` (migration 008, session 48) + `events` (migration 010, session 58) had been deliberately left RLS-off with mitigations applied at different layers — column-validation + service-role-only writes for `site_settings`; GRANT-level `REVOKE SELECT FROM anon, authenticated` (migration 011) + service-role-only writes for `events`. Both choices were defensible at the time, but the Supabase Security Advisor only checks `pg_class.relrowsecurity` and doesn't understand "we protected this a different way." Built [`scripts/security-audit/inspect-rls.ts`](scripts/security-audit/inspect-rls.ts) as a state + behavior probe, ran it against both projects, confirmed identical exposure shape. Other 4 public tables (`malls`, `vendors`, `posts`, `vendor_requests`) were already correctly RLS-on with sensible policies.

**Probe v1 had two bugs caught during diagnostic runs:** (1) DELETE filtered to a sentinel UUID matches zero rows, returns success-with-empty-data — falsely read as "not blocked"; dropped DELETE in favor of INSERT-only write probe. (2) `head: true` count queries to schema-cache-hidden tables returned `n/a` without errors — added explicit error reporting + a TABLE-MISSING verdict for the `events`-might-be-on-staging-or-not case. **Both bugs caught only because the diagnostic was actually run, not just written** — a vindication of script-first over SQL-paste-first.

**Fix arc.** [`supabase/migrations/014_security_advisor_rls.sql`](supabase/migrations/014_security_advisor_rls.sql) — RLS on for both flagged tables with policies matching existing access intent. `site_settings`: re-creates the public-read SELECT policy BEFORE enabling RLS so banners don't break (no reader-visible gap). `events`: enables RLS with zero policies → default-deny for everything except service-role, which bypasses; the existing GRANT REVOKE remains as belt-and-suspenders. **David applied 014 to prod first** ("was that supabase query meant to go into staging or prod because I think I just did it in production") — out of order from the recommended staging-first plan, but the migration was idempotent + low-risk so behavior was unchanged. Verified via direct `pg_class` query in Supabase SQL editor: all 6 tables `rls_enabled = true`. ✅

**Behavioral probe limitation surfaced + closed.** Once 014 landed, my v1 script couldn't distinguish "RLS off + anon-blocked-by-other-means" from "RLS on with no policies" — both produce identical observable behavior at the API surface. [`supabase/migrations/015_security_audit_helpers.sql`](supabase/migrations/015_security_audit_helpers.sql) added `audit_rls_state()` — a SECURITY DEFINER RPC, locked search_path, service-role-only, returns metadata only — that the diagnostic calls to read `pg_class.relrowsecurity` directly. Closes the verification gap permanently. David applied 015 to both projects in-session.

**Streamlined-approach build-out.** David's session opener: *"I want to go through a full security check since we've made some updates and now are pulling in vendor data. I need a process in place to run a security check on supabases especially. I received a few notifications today and need to address these proactively. Let's tackle this and develop a streamlined approach moving forward."* So the work expanded beyond just closing the finding to building reusable audit infrastructure:

- [`scripts/security-audit/inspect-rls.ts`](scripts/security-audit/inspect-rls.ts) — state (via RPC) + behavior (anon SELECT/INSERT) + coverage-drift detection (tables present in DB but not in expectation list trigger ⚠️ unexpected). Falls back to behavioral-only with a loud warning if RPC unavailable.
- [`scripts/security-audit/inspect-storage-acls.ts`](scripts/security-audit/inspect-storage-acls.ts) — bucket public/private state + sample listing + coverage drift for buckets too. Runs purely off the Supabase storage API, no extra RPC.
- [`scripts/security-audit/inspect-keys.ts`](scripts/security-audit/inspect-keys.ts) — repo secret-exposure audit. Verifies `.env*.local` files are gitignored AND not tracked, greps every tracked file for the literal `SUPABASE_SERVICE_ROLE_KEY` value, greps source files for hardcoded `NEXT_PUBLIC_SUPABASE_ANON_KEY` strings. **Substring-match bug** in v1 (`"NOT-TRACKED".includes("TRACKED")` returned true for non-tracked files) caught by running against actual repo state — fixed with distinct YES/NO sentinels + exact equality.
- [`docs/security-audit-runbook.md`](docs/security-audit-runbook.md) — when to run audits (every Advisor email, every milestone, quarterly), what's covered, what's NOT covered yet (function `search_path` mutability, role-grant drift, `auth.users` exposure, OTP/password policy, per-route auth walk all flagged as gaps), step-by-step procedure for handling future Advisor emails, full migration history table.

**Final state in production (as of `50a4ae0`):**

- 3 runtime commits + push, working tree clean.
- **Both Supabase projects:** all 6 public tables `rls_enabled: true`. Security Advisor "Security" tab confirmed clean by David post-paste.
- **Audit infrastructure live:** 3 diagnostic scripts + RPC helper + runbook in repo. Each diagnostic = single CLI command per target. Baseline this session: prod 6/6 PASS, staging 5/5 PASS + `events` table missing on staging (separate infra divergence, not security).
- **Migration 011's GRANT REVOKE on `events` stays intact** — works alongside RLS as belt-and-suspenders.
- **Stale "RLS disabled on all tables" line** in [`MASTER_PROMPT.md`](MASTER_PROMPT.md) §KNOWN PLATFORM GOTCHAS corrected to current state with pointer to the audit diagnostic + runbook.

**Commits this session (3 runtime, smallest→largest):**

| Commit | Message |
|---|---|
| `8b54397` | docs(master-prompt): correct stale RLS gotcha line |
| `669c522` | feat(supabase): close rls_disabled_in_public + audit_rls_state RPC |
| `50a4ae0` | feat(security-audit): diagnostic scripts + runbook for ongoing posture audits |
| (session close) | docs: session 84 close — security audit infrastructure + RLS exposure closed end-to-end |

**Migrations applied to BOTH projects this session (HITL paste):**

- `014_security_advisor_rls.sql` — prod: in-session by David; staging: in-session.
- `015_security_audit_helpers.sql` — both projects in-session.

**What's broken / carried (delta from session 83):**

- 🟢 RLS exposure closed end-to-end. Security Advisor "Security" tab confirmed clean post-paste.
- 🟢 Audit infrastructure live + the runbook is the durable artifact. Next Advisor email = single `npx tsx scripts/security-audit/inspect-rls.ts` command + matching migration.
- 🟡 **Staging missing `events` table** — migration 010 was never applied to staging. Inspector correctly flagged TABLE-MISSING. Decide whether to backfill staging or accept staging-trails-prod policy. Not security; infrastructure divergence.
- 🟡 **Staging missing both Storage buckets** (`post-images`, `site-assets`) — same shape. Manual create via dashboard, or policy decision.
- 🟡 **Audit gaps the runbook flagged but didn't build:** function `search_path` mutability check (Supabase Advisor's `function_search_path_mutable` category), role-grant drift, `auth.users` exposure, OTP/password policy, per-route auth walk. Each is a single-axis Advisor category. ~30–60 min each as standalone or ~2 hrs bundled.
- 🟡 **Phase 3 scroll-restore on /my-shelf + /shelf/[slug] + /flagged** — STILL the longest-standing user-facing carry-forward (session 79's primary user ask, now 5 sessions out). Even more justified now that visual + security layers are settled.
- 🟡 **paper.png in /public** (session 83 carry) — still 2.7MB unused asset from postcard PoC; cleanup candidate.
- 🟡 **3 inline "More from this shelf" code comments** on /find/[id] (session 83 carry) — documentation rot.
- 🟡 **Three near-identical polaroid wrappers** across FindTile + WindowTile + ShelfTile + ShelfCard (session 83 carry) — shared `<FindTile>` primitive extraction trigger pending.
- 🟡 All session 78–83 carry-forwards still hold unless explicitly resolved above.

**Memory updates:** none new this session. The patterns that surfaced (state + behavior probes; coverage drift as a finding; match the advisor's check axis; idempotent migrations for either-order applicability; substring-match bug class) are captured in the runbook §Operating principles + the script header comments at the right scope (project-specific security audit, not generalizable enough to warrant memory files yet).

**Existing memories that load-bore the session:**

- `feedback_treehouse_no_coauthored_footer.md` — applied to all 3 runtime commits ✅.
- `feedback_visibility_tools_first.md` — fired throughout the diagnostic-builder arc; the inspect-rls.ts probe was effectively a "visibility tool" before any migration touched DB state.
- `feedback_kill_bug_class_after_3_patches.md` — fired implicitly when probe v1 had two bugs (DELETE false-positive + substring-match); rather than patch I rewrote the affected slices.
- "Smallest→largest commit sequencing" — **8th clean firing.** docs change → migrations → audit infrastructure. **Promotion-ready and badly overdue.**

**Live discoveries (single firings, Tech Rule candidates):**

- **"State + behavior probes together, neither alone catches all drift"** — first firing as a system principle (captured in `inspect-rls.ts` header + runbook §Operating principles). Behavioral probe couldn't distinguish RLS-off+REVOKE from RLS-on+no-policies; state probe via RPC closed the gap. Tech Rule candidate.
- **"Coverage drift is itself a finding"** — first firing (captured in runbook). Tables/buckets in DB but not in expectation list trigger ⚠️ unexpected; better than silent skip. Tech Rule candidate.
- **"Match the advisor's check axis, not a proxy diagnostic"** — first firing. We could have built a "looks comprehensive" diagnostic that doesn't read `pg_class.relrowsecurity` and the advisor would still flag us. Tech Rule candidate.
- **"Idempotent migrations only — designed for either-order applicability"** — David applied 014 to prod when plan said staging-first; no harm because the migration was idempotent + low-risk. Tech Rule candidate (or merge with existing idempotent-migration norm).
- **"Substring matching on string sentinels is a bug class"** — caught in inspect-keys.ts v1 (`"NOT-TRACKED".includes("TRACKED")` → true). Use distinct tokens (YES/NO) + exact equality. Mostly self-evident but worth flagging.

**Operational follow-ups:**

- **NEW: staging `events` table missing** — migration 010 needs apply, or accept policy "staging trails prod on events instrumentation." Decide.
- **NEW: staging Storage buckets missing** (`post-images` + `site-assets`) — manual create via dashboard, or policy decision.
- **NEW: audit gaps to close** — function `search_path` + role grants + auth.users + OTP/password policy + per-route auth walk. Bundle into one ~2hr session or scatter across milestones.
- **Promote Tech Rule batch (very overdue)** — smallest→largest sequencing now **8 firings**, structural-mockup-axes **4 firings**, mid-session-iPhone-QA **3 firings**, Lora-lineHeight-1.4-minimum **2 firings**, recurring-phrase-signals-system **2 firings**. Plus 4 new single-firings from this session. ~30 min ops pass.
- **Operational backlog roll-up** — paper.png decision, IM Fell + "more from this shelf" inline comments, V1 mockup file tidying, archive-drift backfill (sessions 54 + 55 + 57–83 still missing tombstones in [`docs/session-archive.md`](docs/session-archive.md)), Q-014 Metabase planning, `v1.cartographicBg` semantic token decision.
- All session 78–83 carry-forwards still hold unless explicitly resolved above.

**Notable artifacts shipped this session:**

- [`scripts/security-audit/inspect-rls.ts`](scripts/security-audit/inspect-rls.ts) — state + behavior RLS probe (~270 LOC).
- [`scripts/security-audit/inspect-storage-acls.ts`](scripts/security-audit/inspect-storage-acls.ts) — bucket public/private audit (~190 LOC).
- [`scripts/security-audit/inspect-keys.ts`](scripts/security-audit/inspect-keys.ts) — repo secret-exposure audit (~145 LOC).
- [`supabase/migrations/014_security_advisor_rls.sql`](supabase/migrations/014_security_advisor_rls.sql) — applied to both projects.
- [`supabase/migrations/015_security_audit_helpers.sql`](supabase/migrations/015_security_audit_helpers.sql) — applied to both projects.
- [`docs/security-audit-runbook.md`](docs/security-audit-runbook.md) — cadence + procedure + gaps + migration history.
- [`MASTER_PROMPT.md`](MASTER_PROMPT.md) §KNOWN PLATFORM GOTCHAS — corrected stale RLS line, added pointer to runbook.
...
> Older tombstones live in [`docs/session-archive.md`](docs/session-archive.md). Sessions 28–43 still awaiting archive-drift backfill (one-liner only); sessions 44–53 + 56 in archive (54 + 55 + 57–83 still missing — operational backlog growing). Session 78 fell off the bottom of last-5 visible tombstones at session 84 close; session 79 mini-block rotated off the visible list (still in tombstones list).
...
## CURRENT ISSUE
> Last updated: 2026-04-29 (session 84 close — pure-security session triggered by Supabase Security Advisor `rls_disabled_in_public` email; closed exposure on `site_settings` + `events` across both projects via migration 014; built reusable security audit infrastructure: 3 diagnostic scripts + `audit_rls_state()` RPC (migration 015) + runbook. 3 runtime commits, smallest→largest 8th clean firing. David confirmed Security Advisor "Security" tab clean post-paste.)

**Working tree:** clean. **Build:** green. **Beta gate:** technically unblocked since session 50; no code-level regressions. **Net change:** 3 runtime commits + close. Both Supabase projects now show all 6 public tables `rls_enabled: true` (verified via `audit_rls_state()` RPC). Security Advisor email cleared. **Audit infrastructure live in repo:** [`scripts/security-audit/inspect-rls.ts`](scripts/security-audit/inspect-rls.ts) (state + behavior + coverage drift), [`scripts/security-audit/inspect-storage-acls.ts`](scripts/security-audit/inspect-storage-acls.ts) (bucket public/private), [`scripts/security-audit/inspect-keys.ts`](scripts/security-audit/inspect-keys.ts) (`.env*` gitignore + service-role key leak detection), and [`docs/security-audit-runbook.md`](docs/security-audit-runbook.md) defining cadence + procedure + known gaps. Each diagnostic = single CLI command per target (`prod` | `staging`). **The `/my-shelf` back-nav scroll anchor (session 79 primary user ask) is now THE longest-standing user-facing carry-forward — 5 sessions out, even more justified now that visual + security layers are settled.**

### 🚧 Recommended next session — Re-attempt Phase 3 scroll-restore on /my-shelf + /shelf/[slug] + /flagged (~30–60 min)

David's session 79 primary user ask, carried forward unchanged through sessions 80–84. Phase 3 was reverted in session 79 because it was bundled with morph (layoutId) changes that fought React batching + framer projection + Next.js Link timing. The scroll-restore code itself was preserved in git history (commits `71a0555` + `51067b7`). The session 79 lesson: re-attempt with **STRICT scope** — only module-scope post caches + sessionStorage scroll keys + `requestAnimationFrame` scroll-restore. **NO motion.div / layoutId changes** under any circumstances.

**Why now (over the alternatives):**
- 5 sessions of carry-forward without resolution. David asked for this scroll behavior, didn't get it, has waited.
- Real seeded content (15 posts, since session 82) makes scroll behavior exercisable in iPhone QA — the bug class needs scrollable content to show.
...
- Session 84's security-only pass means the data layer is now also settled — RLS posture is correct, audit infrastructure is in place, advisor email won't fire on this class again.
...
1. **Audit gap follow-ups (security continuation).** The runbook flagged 5 categories not yet built: function `search_path` mutability (Supabase Advisor's `function_search_path_mutable` category), role-grant drift, `auth.users` exposure, OTP/password policy, per-route `/api/*` auth audit. Each is ~30–60 min standalone or ~2 hrs bundled. Strongly justified given we're now collecting real vendor data.
...
3. **Tech Rule promotion batch (badly overdue).** Smallest→largest commit sequencing now has **8 clean firings** — clearly load-bearing across every David-redirect-bundle session AND the security session. `feedback_mockup_options_span_structural_axes.md` has **4 firings**. Mid-session iPhone QA pattern has **3 firings**. Lora-lineHeight-1.4-minimum has **2 firings**. recurring-phrase-signals-system has **2 firings**. Plus 4 new single-firings from session 84 (state + behavior, coverage drift, match advisor's check axis, idempotent-for-either-order). ~30 min ops pass to land them in [`docs/tech-rules-queue.md`](docs/tech-rules-queue.md).

Full alternatives + operational backlog in [`docs/queued-sessions.md`](docs/queued-sessions.md).

### Session 85 opener (pre-filled — Phase 3 scroll-restore re-attempt)
...
CURRENT ISSUE: Re-attempt Phase 3 scroll-restore on /my-shelf + /shelf/[slug] + /flagged. Session 79's primary user ask, reverted because morph (layoutId) changes were bundled with scroll-restore — the scroll-restore code itself worked but the morph integration fought React + framer + Next.js Link timing through 4 iPhone-QA cycles. Reference commits `71a0555` + `51067b7` in git history for the scroll-restore code that worked. STRICT SCOPE: only module-scope post caches + sessionStorage scroll keys + `requestAnimationFrame` scroll-restore. NO motion.div / layoutId changes. NO Track D extension. The Track D infrastructure (preview-cache pattern, layoutId on feed→/find/[id] + /flagged→/find/[id], BoothLockupCard primitive, polaroid tile pattern) all stays untouched. Just scroll behavior. Real seeded content (15 posts) makes this exercisable in iPhone QA. ALTERNATIVES IF SCROLL-RESTORE DEFERRED: (1) Audit gap follow-ups — function search_path mutability, role-grant drift, auth.users exposure, OTP/password policy, per-route /api/* auth audit. Each ~30–60 min standalone or ~2 hrs bundled. Strongly justified post-seeding. (2) Q-014 Metabase analytics surface; decide whether to rename mall_activated/mall_deactivated event keys to location_* as part of Q-014 (backwards-compat migration). (3) Tech Rule promotion batch — smallest→largest sequencing (8 firings + overdue), structural-mockup-axes (4 firings), mid-session-iPhone-QA (3 firings), Lora-lineHeight-1.4-minimum (2 firings), recurring-phrase-signals-system (2 firings) + 4 session-84 single-firings (state-+-behavior, coverage-drift, match-advisor's-check-axis, idempotent-for-either-order). ~30 min ops pass. CARRY-FORWARDS FROM SESSIONS 78–84: preview cache pattern (sessionStorage `treehouse_find_preview:${id}` + module-scope `cachedFeedPosts`) is reusable for any future shared-element work; framer-motion layoutId is a global namespace; testbed `/transition-test` should be USED for surface-extension work; session 80 retired Track D phase 5 source layoutId on /shelves; wordmark on every masthead is TNR upright ink-black 18px (FONT_NUMERAL); /shelves is row pattern not grid; bookmark on BoothHero photo corner not in masthead; IM Fell COMPLETELY RETIRED (FONT_LORA token); BoothLockupCard shared primitive owns /shelves + /flagged; Polaroid evolved tile pattern owns Home + /flagged + /shelf/[slug] WindowTile + ShelfTile (warm cream `#faf2e0` paper, 4px radius, 7px photo mat, dual-shadow); Title+price on tiles is centered Lora 14px group (matches /find/[id] hero pattern at scale), height locked at 76 (FindTile/WindowTile/ShelfTile) or 56 (ShelfCard strip); /find/[id] strip got centering WITHOUT polaroid wrapper per browse-vs-navigate rule. **NEW from session 84: RLS enabled on every public table (migrations 014+015 applied to both projects); audit_rls_state() RPC live as service-role-only SECURITY DEFINER for state-aware diagnostics; security-audit infrastructure under scripts/security-audit/ + docs/security-audit-runbook.md as durable institutional memory; before pasting any future Supabase migration, follow the runbook's standard procedure (script-first, staging-then-prod, idempotent SQL, verify with the diagnostic). Staging is missing the `events` table + both Storage buckets — infrastructure divergence, not security; decide policy.**
```
