...
| D10 | Mall name typography | **Lora 26 / 500 / inkPrimary / line-height 1.15** with `WebkitLineClamp: 2`. Up to 2 lines for long names; 26px (down from V2's 28) to leave room for the square stamp on the right. | V4 dial after stamp went square. |
| D11 | Address row | **Lucide MapPin 13 (inkMid) + system-sans 13 (inkMid).** Single line, ellipsis. Composes from `address + city + state + zip`. | V2 → V4. |
...
| D19 | Tap behavior on the card | **Opens `<MallSheet>`** (existing primitive). The card IS the mall picker — tapping it lets the user change scope. Same UX as today's MallScopeHeader. | Session 106 — preserves existing pattern. |
| D20 | Search bar placement on tab pages | **Below the mall card, above the body content.** Matches R16's session-105 D2 reversal (search subordinate to mall scope). Same `<SearchBar>` primitive shipped session 105. | Sessions 105 + 106 — direct reuse. |
| D21 | Search bar placeholder copy | **"Search finds, booths, or styles"** — adds "or styles" to surface R16's tag layer. Was "Search finds, booths…" prior. | David, session 106. |
...
| D27 | Search bar behavior on Map | **Redirect to Home.** Submitting a search on Map routes to `/?q=<query>` preserving any active mall scope via existing URL params. Search lives on Home (R16); Map stays focused on geographic browse. Search-bar primitive itself stays visible on Map for cross-tab consistency (D20). Filter-pins-in-place rejected: requires aggregation infra not in current schema. Disabled placeholder rejected: dishonest UI. | V5 mockup, David accepted recommendation session 106. |
