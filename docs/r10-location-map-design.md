...
|---|----------|----------|--------|
| D1 | Where does the Map slot in the BottomNav? | ~~**New 5th tab → reduce to 4 tabs by retiring Booths.** Final order: Home / Map / Profile / Saved.~~ **REVERSED session 109** → 3-tab flat: Home / Map / Saved. Profile relocates to masthead left slot via `<MastheadProfileButton>` (mirrors back-button geometry on detail pages). **REVERSED AGAIN session 110** → 2-tab flat: Home / Saved. Map retired from nav too. Reasoning per David: "the two ways to get to the map view a) from the location component up top and b) map icon in nav feel disjointed now." /map becomes an interaction surface (pick scope, return to feed) accessed via the postcard mall card on Home/Saved — not a destination. Nav structure is now 3 reversals deep on D1 (107 lock → 109 → 110). | David's hand-drawn mockup, session 106 + David's session-109 iPhone refinement + David's session-110 iPhone refinement. |
...
| D19 | Tap behavior on the card | ~~Opens `<MallSheet>` everywhere — the card IS the mall picker on every tab.~~ ~~**REVERSED (partial) session 107** → On **Map** the card opens MallSheet (the change UI). On **Home + Saved** the card routes to `/map` instead.~~ **FULLY REVERSED session 110** → On **Map** the card is also no-op (informational only); affordances live on the new contextual pill (top-right of map). On **Home + Saved** the card still routes to `/map`. PostcardMallCard primitive updated to render as a non-interactive `<div>` (cursor: default) when no `onTap` is provided, so the /map card doesn't look tappable. The card's "from: <mall>" identifier role stays universal across surfaces. | David, session 107 + 110 iPhone QA. |
...
| D26 | Pin-tap interaction | **Peek-then-commit.** Tap a pin → pin enters selected state (green fill, glyph in `green-on`, scale +15%, soft halo) AND an inline `<PinCallout>` appears above the pin showing 36px hero thumb + Lora mall name + Times-New-Roman booth/find counts + chevron. Tap the callout → commits the rescope (postcard card animates to that mall, map zooms in to specific-mall state). Tap empty map → dismisses callout, pin returns to default. Refines D23's earlier instant-rescope language. **EXTENDED session 110** → after `setMallId(id)` fires, `router.push('/')` so the user lands on the feed for the mall they just picked. /map is an interaction surface (pick scope, return to feed), not a destination. Per David: *"on the second tap it should go to the home screen to view the feed."* MallSheet selection from the new contextual pill deliberately STAYS on /map (asymmetric by design — list browsing keeps browsing context). | V5 mockup, David accepted recommendation session 106 + David's session-110 iPhone refinement. |
...
## Session 110 nav refinement

David's session-110 ask reframed /map from a primary destination into an interaction surface (pick scope, return to feed). Four runtime commits + this docs commit. Focused on closing the "two paths to /map feel disjointed" gap by collapsing them to one path (postcard mall card on Home/Saved), and giving /map dedicated affordances for clearing scope and browsing the mall list.

| Commit | Ship | Decision impact |
|---|---|---|
| `931c389` | Drop Map from BottomNav (3-tab → 2-tab) | **D1 second reversal.** Nav becomes Home / Saved. /map reached only via postcard card tap. |
| `532fa2e` | PostcardMallCard.onTap no-op on /map | **D19 fully reversed.** Card is informational on /map. PostcardMallCard primitive updated to render as `<div>` (not `<button>`) when no `onTap` provided. |
| `c889050` | Pin commit routes to Home after `setMallId` | **D26 extended.** Pin tap → callout → tap callout → setMallId + `router.push('/')`. /map is interaction, not destination. |
| `d3d5f5e` | Contextual pill on /map + MallSheet relocated from layout | New affordance. Single state-aware pill: 'Clear' (+ X-glyph) when scope set; 'Browse list' (+ List glyph) when all-Kentucky. MallSheet hosted on /map page now (only opener). |

### Session 110 implementation notes

- **Nav reduction logic:** with only Home + Saved in the BottomNav, both tabs go AWAY from /map. That's by design — both are "exit /map" buttons in /map's context, which matches the new mental model where /map is a brief interaction surface.
- **PostcardMallCard interactivity contract:** the primitive now renders as `<button>` only when an `onTap` prop is provided. When `onTap` is undefined, renders as `<div>` with `cursor: default` (no button semantics, no tap highlight). This avoids the "looks tappable, does nothing" UX that omitting the handler caused before. `interactive` is detected via `typeof onTap === "function"`.
- **Pin commit vs MallSheet selection asymmetry:** pin commit auto-routes to Home; MallSheet selection stays on /map. Per David's Q3 — list browsing keeps the browsing context (the user wants to see other malls), pin tap is a deliberate "I want this one" action. Worth preserving even though it's slightly inconsistent with pin commit's auto-route.
- **MallSheet relocation:** MallSheet hosted in `app/(tabs)/layout.tsx` at session 109 had only one opener (PostcardMallCard.onTap on /map). Session 110 retired that opener; MallSheet now lives at `app/(tabs)/map/page.tsx` next to its only consumer (the contextual pill). Removed orphan-state risk.
- **Contextual pill:** single state-aware affordance per David's Q2='E' pick. Top-right inside map container at `top: 12, right: 12, z-index: 5`. v1.green pill with v1.onGreen text/icon, FONT_SYS 13px/600. Lucide X glyph (15px stroke 2.0) for clear, Lucide List for browse. Uses `v1.shadow.callout` to lift over the basemap.
- **R3 analytics — `source` field added to `filter_applied`:** distinguishes scope-change paths on /map: implicit (no source field — pin commit), `clear_pill`, `list_pick`, `shared_url` (added session 109).
