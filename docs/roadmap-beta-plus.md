...
| R5a | 30-day feed window | Feed quality | S | — | ✅ Shipped session 91 | `getFeedPosts` adds `gte("created_at", cutoff)` where cutoff = now − 30 days. `/my-shelf`, `/shelf/[slug]`, `/flagged` intentionally exempt — only the public discovery feed. |
