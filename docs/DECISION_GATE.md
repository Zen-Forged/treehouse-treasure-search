...
> Last updated: 2026-04-21 (session 43 — Anthropic model audit + billing safeguards. Session-27 `Anthropic model audit` Tech Rule fired for the first time since promotion, triggered by Anthropic's April 16, 2026 `claude-opus-4-7` announcement. Blind grep of `app/` + `lib/` surfaced four Anthropic call sites (`/api/post-caption` + `/api/story` on `claude-sonnet-4-6`, `/api/identify` on `claude-opus-4-7`, `/api/suggest` on `claude-opus-4-6`) — all three model strings confirmed Active on the Anthropic deprecations page with 10+ month retirement runways. No code swap required; session-27 remediation is holding cleanly. Console auto-reload enabled at threshold $10 / reload $20 — structurally closes the silent-failure surface the session-27 `Anthropic billing as silent dependency` rule was written to mitigate. Two Risk Register rows flipped below.)
...
| `/shelves` `AddBoothSheet` will be orphaned after T4b | 🟢 Low | ✅ Resolved session 37 — T4b fold-in shipped. `AddBoothSheet` deleted from `/shelves`; `<AddBoothInline>` primitive added to `/admin` Vendors tab (v1.1k chrome). `/shelves` is now a browse-only surface. | Dev agent |
...
> Last updated: 2026-04-21 (session 43 — Anthropic model audit + billing safeguards; Risk Register rows flipped; no code, no new rules promoted)
