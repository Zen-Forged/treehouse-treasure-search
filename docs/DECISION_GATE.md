...
Two Tech Rules are queued for promotion from recent session observations. Both would land in the block above; both are waiting on a dedicated Tech Rule batch session so they get the same careful prose treatment as the rest.
...
Promote both together as a batch when there's session budget for prose treatment.
...
> Last updated: 2026-04-20 (session 35 close — KI-006 ✅ Resolved and multi-booth ✅ Resolved; two new Q-002/Q-003 non-gating deferrals logged; two Tech Rule candidates queued for batch promotion)
...
| Sprint 4 | Beta-readiness — custom domain, OTP auth, `/admin` polish, vendor onboarding | 🔄 In progress. Shipped: T1/T2/T3/T4a, KI-001/002/003/004, `/setup` 401 polish, T4c partial, session-32 v1.2 onboarding refresh, session-35 multi-booth rework + KI-006 fix. Remaining: T4c copy polish (orphans C+D), T4b (admin surface consolidation), T4d (pre-beta QA pass). |
...
> Last updated: 2026-04-20 (session 35 — multi-booth rework shipped; KI-006 resolved; two Tech Rule candidates queued for batch promotion; Q-001 retired, Q-002/Q-003 logged)
