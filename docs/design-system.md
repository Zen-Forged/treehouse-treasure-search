...
> Version: 1.1l | Last updated: 2026-04-18 session 24 | Owned by: Design agent
...
**v1.1l — Featured banners + sticky-masthead chrome + on-device polish (session 24).** A mixed refinement pass from David's on-device walk of v1.1k. Two new primitives, one shared chrome primitive, one glyph-hierarchy addition on Find Map, and one deliberate partial reversal of v1.1j's post-it font commitment. Seven doc-level changes land with this pass:

(a) **`<StickyMasthead>` primitive — scroll-linked hairline border.** All six mastheads in the product (Home, Find Map, Find Detail normal, Find Detail 3B, My Shelf, Public Shelf) previously each held their own inline `position: sticky` implementation with the bottom hairline always visible. On device the hairline at rest reads as a separator between two already-different surfaces (nothing above, paper below), which is redundant chrome — the separator only becomes useful once content is scrolling under the masthead. v1.1l extracts `<StickyMasthead>` as a shared component (`components/StickyMasthead.tsx`) that carries: `position: sticky, top: 0, zIndex: 40`, paperCream-96%-translucent background with `backdrop-filter: blur(24px)`, and a **scroll-linked bottom border** that fades from `transparent` → `1px solid v1.inkHairline` when `window.scrollY > 4`. The transition uses `border-color 0.2s ease` so the reveal reads as a soft threshold crossing, not a snap. All six consumers migrate to the primitive in this sprint; any future sticky masthead uses the primitive, never a hand-rolled equivalent.

(b) **`<FeaturedBanner>` primitive — admin-editable hero banner.** A new primitive for hero-scale decorative banners with two variants: `eyebrow` (title sits above the image as a separate text block; used on Home as "Featured Find") and `overlay` (title renders on top of the image in IM Fell 30px white with a `0 2px 8px rgba(0,0,0,0.35)` text-shadow for legibility against any photograph; used on Find Map as "Find Map"). Common composition: 16px corner radius (inherits from the banner radius token — matches Booth hero), full-width within 10px page padding (10px is the Booth banner's committed inset; reused here for consistency), ~200px minimum height for Home (Featured Find sits inside the natural feed scroll) and ~180px minimum height for Find Map (shorter because it carries the page title and sits above content-heavy itinerary). Subtle top-down scrim (`rgba(18,34,20,0.32)` → transparent, 40% height) only on the overlay variant — the eyebrow variant doesn't need it. Image is a single admin-uploaded photograph served from a new Supabase Storage bucket `site-assets`; no caption, no link target in v1 (purely decorative per the (c) simplest-data-model commitment). Primitive file: `components/FeaturedBanner.tsx`.

(c) **Featured-banner data model — `site_settings` keyed row.** A new `site_settings` table in Supabase holds a single row keyed by setting name (string primary key) with a JSON `value` column. Initial keys: `featured_find_image_url` (Home) and `find_map_banner_image_url` (Find Map). Admin-only writes via a new `/api/admin/featured-image` route that (1) requires admin auth via `requireAdmin`, (2) accepts a base64 data URL and setting key, (3) uploads the image to the `site-assets` bucket via service role client, (4) upserts the resulting public URL into `site_settings`. Client-side reads are direct Supabase reads (no RLS on `site_settings` — public read, service-role write); the image URL is fetched server-side on page load and hydrated into the `<FeaturedBanner>` as a prop. No image in the DB = no banner renders (the page collapses gracefully; no placeholder chrome). The decision to defer a `link_url` / `caption` / `featured_post_id` to a future version is deliberate: the primitive ships useful on day one, and graduating to a linked-post or captioned variant is additive work that doesn't require re-architecture.

(d) **Find Map — glyph weight + copy + layout polish (five sub-items from on-device walk):**
  - **Spine line connects the last stop to the closer circle.** The v1.1j closer spine drops 48px from empty space to a terminal circle — on device with real content above it, the drop reads as disconnected from the last stop. Fix: the hairline tick that the `<Stop>` component renders between stops now also renders *beneath the last stop* (to join it to the closer), and the closer's 48px spine drop starts from where the last stop's spine ends. The visual result is one continuous hairline from the mall-anchor pin all the way down to the terminal circle, with X glyphs along the path.
  - **Closer copy aligned vertically with the terminal circle.** v1.1j put the closer copy below the circle in a separate grid row; on device this reads as *circle, then pause, then sign-off*. v1.1l moves the copy into the same grid row as the circle, right-column, vertically centered. Composition: `26px | 1fr` grid, circle in left column, copy in right column aligned with `alignItems: center`. The reading becomes *the path ends here, and here's what that means* — one thought, not two.
  - **Closer copy changes to the tagline.** "End of the map. Not the end of the search." retires in favor of **"Embrace the Search. Treasure the Find."** — a two-thirds surfacing of the product's tagline anchor (`docs/DECISION_GATE.md` The Vision). "Share the Story" is deliberately omitted because the Find Map page is about *the hunt*, not *the telling* — the third clause would dilute the cadence. This is the first place in the product where the tagline surfaces as page copy (rather than existing only in the design-system anchor). Font/size/ink unchanged (FONT_SYS 15px inkMid).
  - **X glyph weight bumps to match the terminal circle's anchor weight.** v1.1g/j shipped the X at `strokeWidth=1.5` — next to the 16px filled-circle terminal, the X reads as lighter than the circle, breaking the spine's visual rhythm. Fix: X `strokeWidth` goes 1.5 → 2.2 (on both the Find Map itinerary spine X AND the Find Detail vendor-row X, so the two surfaces stay consistent). No other X dimension changes. The Find Detail cartographic block's X (15px) and Find Map's itinerary X (18px) both pick up the bolder stroke.
  - **Vendor name italic retires on Find Map stops.** Shipping the vendor name in IM Fell italic 18px alongside the non-italic mall name (also IM Fell, 22px) created an inconsistency that read as *mall is a place, vendor is a voice* — but both are *places* in the cartographic reading. Retire italic on the vendor name; now IM Fell 18px non-italic mid-ink. Italic is reserved on this page for the intro paragraph and for the absent-booth fallback text ("No booth listed").

(e) **Post-it 36px booth numeral — partial v1.1j reversal to Times New Roman.** v1.1j swapped the 36px numeral on the post-it from `FONT_IM_FELL` → `FONT_SYS` to fix the "1 vs I" disambiguation bug (IM Fell's single-serif `1` read as capital-I). That fix held — but on David's on-device walk the `FONT_SYS` numeral at 36px reads as *system chrome on a post-it*, breaking the material gesture the note is meant to carry. v1.1l commits a **narrow exception**: the 36px numeral on the post-it goes to `Times New Roman, Times, serif` (a broadly-available system serif with a clearly-disambiguated `1`, no extra Google Fonts load, no editorial personality that would clash with IM Fell). **The exception is only for the 36px numeral on the post-it** (Find Detail + Booth banner). Inline pills on vendor rows (Find Detail `<Pill>`, Find Map `<BoothPill>`) stay on `FONT_SYS` per v1.1j — at those small inline sizes against short number strings the 1-vs-I ambiguity is still real, and the label context ("Booth" word adjacent) already disambiguates the reader. The exception earns its exception: the post-it is a *material gesture* where serif matters; the pill is *data-in-chrome* where system clarity matters. Committed token name: `FONT_POSTIT_NUMERAL` in `lib/tokens.ts`.

(f) **Post-it 36px numeral auto-scales by digit count.** Long booth numbers (5–6 digits) overflow the 92×92px post-it at the committed 36px size. Rather than widen the post-it (which would ripple into the banner composition on both Find Detail and Booth) or cap booth numbers (which would misrepresent reality — real booths can have 4–5 digits), v1.1l commits an auto-scaling table: **36px for ≤4 digits, 28px for 5 digits, 22px for 6 digits**. Letter-spacing stays at `-0.01em` for all sizes. 4-digit is the typical case; 5-digit handles most long booths; 6-digit is the ceiling before the post-it would need to grow. Applied in both `app/find/[id]/page.tsx` and `components/BoothPage.tsx` via a shared helper in `lib/utils.ts:boothNumeralSize(boothNumber)`.

(g) **BottomNav idle icon color darkens to match v1.1 ink scale.** v1.1d's BottomNav minimal patch left the idle (inactive) tab color at the legacy `#8a8476` (v0.2 `textMuted`). Against the paperCream background this reads as *too muted* — the inactive Home and Find Map tabs disappear into the chrome. v1.1l bumps idle color `#8a8476` → `v1.inkMuted` (`#6b5538`), bringing BottomNav into the v1.1 ink scale. Active tab (`#1e4d2b` green) unchanged. This is the first real BottomNav chrome change since v1.1d; the full chrome rework still scopes for the Nav Shelf sprint, but this one-line idle-color fix is too high-value to hold for that sprint.

What shipped across files: **new** `components/StickyMasthead.tsx`, `components/FeaturedBanner.tsx`, `app/api/admin/featured-image/route.ts`, `lib/siteSettings.ts` (reads), `supabase/migrations/[timestamp]_site_settings.sql` (schema). **Modified** `lib/tokens.ts` (`FONT_POSTIT_NUMERAL` added), `lib/utils.ts` (`boothNumeralSize` helper added), `app/page.tsx` (masthead migrated, featured banner wired), `app/flagged/page.tsx` (masthead migrated, featured banner wired with overlay variant, five polish items), `app/find/[id]/page.tsx` (masthead migrated ×2, post-it font swap + auto-scale, X stroke bump), `app/my-shelf/page.tsx` (masthead migrated), `app/shelf/[slug]/page.tsx` (masthead migrated), `components/BoothPage.tsx` (post-it font swap + auto-scale), `components/BottomNav.tsx` (idle color bump), `app/admin/page.tsx` (new Featured Banners tab). Pattern retirement log gains three entries.

**v1.1k — activation flow pass (session 23).** The last four v0.2 surfaces in the ecosystem — `/vendor-request` (form + success), `/login` (email entry + OTP + confirming), `/setup` (linking + success + error), and the PIN entry surface that previously lived as a tab inside `/login` — move forward to v1.1j vocabulary in a single pass. This is also the session that resolves the session-8 onboarding-journey commitment to retire the Admin PIN tab from `/login`; a new dedicated `/admin/login` route lands as part of this pass. Pulled ahead of the Sprint 5 "Curator Sign In rename" bundle because the activation funnel is the *first impression* for every Flow 3 remote vendor and the v0.2 Georgia + `#f5f2eb` chrome was undermining the on-device first-impression work the rest of v1.1i/v1.1j did for shoppers. Eight doc-level commitments land with this pass:

(a) **Mode C resolved for task-first surfaces.** The v1.0 header-pattern system committed Mode C ("back arrow + page title only, no masthead") for `/post`, `/post/preview`, `/vendor-request`, `/setup`, `/login` but didn't specify interior grammar. v1.1k commits: back-arrow paper bubble (`v1.iconBubble` 38px) top-left, **no masthead wordmark**, no diamond dividers, no post-it, no cartographic glyphs. Activation is a task, not a browse — the journal voice names the moments that matter (page title, closing line, headline) and steps back everywhere else. Editorial voice (`FONT_IM_FELL`) carries titles, subheads, and end-of-path copy; precise voice (`FONT_SYS`) carries form fields, inputs, email echoes, timers, and error messages. The voice split is the single most important commitment here — earlier drafts put IM Fell on every surface, which produced a page that read as *reflective when it should have read as functional*.

(b) **Paper-wash success bubble primitive.** The v0.2 `greenLight`/`greenBorder` circle (used on `/vendor-request` success, `/login` "Signing you in…" state, `/setup` success) retires. v1.1k commits a paper-wash variant of the icon-bubble primitive at hero scale: **60px diameter, `rgba(42,26,10,0.04)` background, 0.5px `v1.inkHairline` edge, glyph in `v1.inkPrimary` at 26px**. The checkmark glyph (or other state glyph — spinner for loading, tilde for warning) is the state signal; the bubble is the *gesture of a mark on paper*. This generalizes the v1.1f paper variant icon bubble (38px `v1.iconBubble` for chrome actions) to the 60px success-moment scale while keeping the logic consistent: *the ink carries the state; the shape carries the gesture*. Retires the SaaS success-toast green chrome everywhere it appeared in the activation flow. Rule propagates to any future success-moment surface (confirmation screens, post-publish confirmation).

(c) **Filled green CTA rule — commit actions only.** Filled `v1.green` buttons (white `FONT_SYS` 15px 500 weight label, 14px corner radius, 15×0 padding) are reserved for **commit actions** — actions that create database rows, send emails, authenticate a session, or move a user forward into a working surface. Specifically: "Request access" on `/vendor-request`, "Email me a code" on `/login`, "Sign in as Admin" on `/admin/login`, "Go to my shelf" on `/setup` success. End-of-path actions, alternative paths, retries, resends, and navigation offers become IM Fell italic dotted-underline text links in `v1.inkPrimary`: "Explore the feed →" on `/vendor-request` success, "Go back" on form screens, "Resend code" on OTP entry, "Try again" and "Go back to sign-in" on `/setup` error. Rule propagates: the distinction between commit-action-CTA and end-of-path-text-link is now the committed language across every Mode C surface. Retires the v0.2 instinct to style every primary action as a filled green button regardless of its role.

(d) **Form input primitive.** Committed v1.1k input treatment: **white translucent background `rgba(255,253,248,0.70)`, 1px `v1.inkHairline` border (focus: 1.5px `v1.inkPrimary`; error: 1.5px `v1.redBorder`), 14px corner radius, 14px horizontal and vertical padding, `FONT_SYS` 16px `v1.inkPrimary`**. The white-translucent-on-paper treatment reads as a quiet form field *sitting on* the paper, not a panel cut out of it. 16px input text honors the primary-reading floor (and prevents iOS input zoom-on-focus, a meaningful UX bonus). Labels above fields are IM Fell italic 13px `v1.inkMuted` with natural sentence casing — **no `text-transform: uppercase`, no `letter-spacing` greater than `normal`**. Retires every uppercase-tracked form label across all v0.2 forms (`EMAIL ADDRESS`, `6-DIGIT CODE`, `YOUR NAME`, `YOUR MALL`, `BOOTH NUMBER`, `ADMIN PIN`). The uppercase-tracked label was a v0.2 holdover already retired on the browse side in v1.1 — this sprint just finishes the job on the task side.

(e) **Email echo line primitive.** Committed v1.1k for any surface where the user needs to see *which email got the code/link*: a horizontal row, NOT a surface card. **14px mail glyph `v1.inkMuted` + "Sent to " (`FONT_SYS` 14px `v1.inkMuted`) + email address (`FONT_SYS` 14px `v1.inkPrimary` 500 weight), 12px vertical padding, bordered above and below by 0.5px `v1.inkHairline` rules**. Reads as a shipping-label line — a statement about where the code went, not a panel of data. Appears on `/login` OTP entry and on `/vendor-request` success. Retires the v0.2 surface-card treatment that wrapped this information in a `colors.surface` bg with a rounded border — that was card chrome pretending to be information, the exact pattern v1.0 committed against globally.

(f) **Mode C tab switcher retires.** The rounded-pill `Email code / Admin PIN` tab switcher on `/login` (v0.2) retires entirely. Rationale: rounded-pill tab chrome reads as SaaS dashboard (already established in v1.1h when the Booth `TabSwitcher` retired), AND bundling curator sign-in with admin sign-in on the same route conflated two audiences with meaningfully different needs. **Admin PIN moves to a new dedicated route `/admin/login`**, leaving `/login` as curator-only. The session-8 `docs/onboarding-journey.md` commitment ("Remove 'Admin PIN' tab from `/login`; move behind `/admin` gate") is resolved by this move — the disposition of `/admin/login` vs. folding it into `/admin`'s unauthenticated-gate is a T4b decision, deliberately deferred (see below).

(g) **`/admin/login` scope committed.** New route, dedicated PIN entry surface. Three states: PIN entry (default), `signing-in` (post-verify, waiting on Supabase session), error. Composition follows Mode C: back-arrow paper bubble top-left, logo + "Admin Sign in" title + IM Fell italic subhead "Enter your PIN to continue.", single v1.1k form input (password type, 20px centered text with 0.4em letter-spacing for the password-dot tracking, `FONT_SYS`), filled green "Sign in as Admin" CTA with `Shield` icon inline, paper-wash bubble + spinner in the `signing-in` state, red banner for error. Successful PIN verify → `router.replace("/admin")` (unchanged from current behavior). The `/admin` page itself is **out of scope** for v1.1k — its unauthenticated-gate stays as-is (still redirects to `/login` in the fallback Sign-in button), and when T4b runs admin surface consolidation, it will decide whether to (1) keep `/admin/login` dedicated, (2) fold it into `/admin`'s unauth gate, or (3) remove the `/login`→`/admin` fallback link entirely now that `/admin/login` exists. The STOP rule on auth-flow changes was the deciding factor — bundling admin gate changes into an activation-flow design session would have crossed the DECISION_GATE threshold for multi-system coupling.

(h) **`<MallSheet>` migration to `/vendor-request` deliberately deferred.** The form field "Your mall (optional)" stays as a native HTML `<select>` in v1.1k. Rationale: the `<MallSheet>` primitive committed in v1.1i is the canonical mall selector, but migrating `/vendor-request` to it couples design work (the vendor-request pass) with component work (first non-Feed consumer of MallSheet). The v1.1k scope is already touching four routes; adding MallSheet consumer plumbing to `/vendor-request` in the same sprint widens the blast radius. The explicit deferral: native select styled to match v1.1k form input primitive, with a TODO comment in the code pointing at this Status paragraph. MallSheet migration lands in a dedicated Sprint 5 sub-sprint alongside `/post` + `/post/preview` MallSheet consumers.

What shipped across four files: `app/vendor-request/page.tsx` full rewrite, `app/login/page.tsx` full rewrite (loses PIN tab, `handlePin`, `pin-signing-in` state, `TabMode` type), `app/setup/page.tsx` full rewrite, `app/admin/login/page.tsx` NEW. All four routes retire their inline `colors` / `C` palettes and import from `lib/tokens.ts` (`v1`, `FONT_IM_FELL`, `FONT_SYS`). One surgical edit to `app/admin/page.tsx` — no visual change, just pointing the unauthenticated-gate Sign-in button to `/admin/login` instead of `/login` (optional; could also leave it pointing at `/login` since curators might also land on `/admin` by mistake — to decide during code sprint). Pattern retirement log gains four entries: v0.2 `greenLight` success check-bubble chrome; rounded-pill tab switcher on `/login`; uppercase + tracked-letter-spacing form labels; green info boxes (`Admin-only access…`, `✓ Approved · ready to sign in`) in favor of typography-carries-the-state.
...
| v0.2 `greenLight` / `greenBorder` success check-bubble chrome (v1.1k) | Paper-wash 60px success bubble (`rgba(42,26,10,0.04)` bg, 0.5px `inkHairline` edge, glyph in `inkPrimary`) | Green check-bubble read as SaaS success-toast; the ink carries the state, the shape carries the gesture. Paper-wash bubble matches the v1.1f paper-variant icon bubble primitive at hero scale, keeping the visual logic consistent across chrome actions and success moments |
| Rounded-pill tab switcher on `/login` (v1.1k) | Admin PIN split to dedicated `/admin/login` route; `/login` becomes curator-only | Rounded-pill tab chrome read as SaaS dashboard (already established in v1.1h Booth `TabSwitcher` retirement). Bundling curator sign-in with admin sign-in on one route also conflated two audiences with meaningfully different onboarding contexts; splitting the routes lets each speak only to its own audience. Resolves the session-8 onboarding-journey commitment to retire the Admin PIN tab |
| Uppercase + tracked-letter-spacing form labels (v1.1k) | IM Fell italic 13px `inkMuted` with natural sentence casing — no `text-transform: uppercase`, no `letter-spacing` greater than `normal` | The Title Case commitment from v1.1 applied to browse surfaces in that sprint but left form labels on task surfaces still uppercase-tracked. v1.1k finishes the job. SaaS-dashboard voice has no place in the activation flow |
| Green info boxes on activation flow (v1.1k) | Typography carries the state — subheads name the role, success moments use the paper-wash bubble, error states use the red banner primitive | `Admin-only access. Enter your PIN to sign in instantly.` as a green-tinted info box was belt-and-suspenders after the subhead already named the role. `✓ Approved · ready to sign in` on admin toast similarly redundant. Paper-as-surface + typographic hierarchy + restrained banner primitives carry the meaning without the info-box chrome |
| Always-on masthead bottom hairline (v1.1l) | Scroll-linked hairline via `<StickyMasthead>` primitive — fades from transparent to 1px `inkHairline` once `window.scrollY > 4` | At rest, the hairline was a chrome separator between two already-distinct surfaces (translucent masthead / paper body). The separator earns its weight only when content is scrolling underneath the masthead — then it anchors the overlay. Fading it in/out matches the threshold metaphor (the page becoming a scrollable surface) rather than treating every page as pre-scrolled |
| FONT_SYS on the 36px post-it booth numeral (v1.1l, partial v1.1j reversal) | `FONT_POSTIT_NUMERAL` (Times New Roman, Times, serif) for the 36px post-it numeral only; inline pills stay on FONT_SYS | v1.1j's system-ui numeral at 36px on the post-it read as system chrome on a material gesture, breaking the note's skeuomorphic weight. Times New Roman has a disambiguated `1` (solving v1.1j's original 1-vs-I concern) and doesn't require an extra Google Fonts load. Narrow exception only — the pill context at small inline sizes still benefits from FONT_SYS disambiguation, where the post-it at hero scale with a serif-compatible material doesn't |
| Italic on Find Map vendor name (v1.1l) | IM Fell non-italic 18px mid-ink on vendor names in Find Map stops | Italic vendor name next to a non-italic mall name created a voice inconsistency (mall = place, vendor = voice) that didn't match the cartographic reading (both are places). Non-italic brings the two into parity. Italic is reserved on Find Map for the intro paragraph and the absent-booth fallback |
| BottomNav idle color `#8a8476` (v1.1l) | `v1.inkMuted` (`#6b5538`) | Legacy muted read as *disappeared* against paperCream on device. Bringing idle into the v1.1 ink scale restores visible tab weight without touching active-state green or structure. One-line fix held for speed; full BottomNav chrome rework still scopes for Nav Shelf sprint |
...
| `<MallSheet>` migration to `/post`, `/post/preview`, and `/vendor-request` | Sprint 5 dedicated sub-sprint. `/vendor-request` was deliberately deferred in v1.1k to keep that sprint surgical; the field currently uses a styled native `<select>` with a TODO pointing at the v1.1k Status paragraph (h). First non-Feed consumer lands with this sprint |
...
> Last updated: 2026-04-18 (session 24 — v1.1l Featured banners + sticky-masthead chrome + on-device polish: `<StickyMasthead>` + `<FeaturedBanner>` primitives committed; `site_settings` table + `/api/admin/featured-image` added; Find Map spine continuity + closer vertical alignment + tagline surfacing + X stroke bump + vendor-name italic retire; post-it 36px numeral partial v1.1j reversal to Times New Roman with auto-scale-by-digit-count; BottomNav idle color aligned to v1.1 ink scale)
