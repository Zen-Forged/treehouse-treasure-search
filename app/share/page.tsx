...
export default function SharePage() {
  ...
  const caption = session?.captionRefined ?? session?.intentText ?? "";
  ...
  const handleSave = () => {
    if (!session) return;
    saveFind(sessionToFind(session, "shared"));
    setSaved(true);
    clearSession();
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#050f05]">
      ...
      <main className="flex-1 flex flex-col px-5 py-5 gap-5 pb-40 overflow-y-auto">
        ...
        <motion.div className="w-full rounded-2xl overflow-hidden flex-shrink-0" style={{ height: 220, border: "1px solid rgba(109,188,109,0.08)" }}
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.45 }}>
          {image && <img src={image} alt="Your find" className="w-full h-full object-cover" style={{ filter: "brightness(0.9) saturate(0.85)" }} />}
        </motion.div>
        ...
        {title && (
          <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.08, ease }}>
            ...
            <div style={{ fontFamily: "Georgia, serif", fontSize: 17, fontWeight: 600, color: "#f5f0e8" }}>{title}</div>
          </motion.div>
        )}
        ...
        <motion.div className="flex flex-col gap-2" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2, ease }}>
          <div style={{ fontSize: 9, color: "#7a6535", textTransform: "uppercase", letterSpacing: "2px", marginBottom: 2 }}>Share</div>

          <motion.button onClick={handleFacebook} className="w-full flex items-center justify-center gap-2"
            style={{ padding: "14px 18px", borderRadius: 14, fontSize: 13, fontWeight: 500, background: posted ? "rgba(45,125,45,0.12)" : "rgba(13,31,13,0.5)", color: posted ? "#6dbc6d" : "#d4c9b0", border: posted ? "1px solid rgba(109,188,109,0.25)" : "1px solid rgba(109,188,109,0.1)", transition: "all 0.2s", cursor: "pointer", fontFamily: "inherit" }}
            whileTap={{ scale: 0.97 }}>
          ...
          <div className="flex gap-2">
            <motion.button onClick={handleCopy} className="flex-1 flex items-center justify-center gap-2"
              style={{ padding: "13px 8px", borderRadius: 12, fontSize: 12, fontWeight: 500, background: "rgba(13,31,13,0.5)", color: copied ? "#6dbc6d" : "#d4c9b0", border: copied ? "1px solid rgba(109,188,109,0.3)" : "1px solid rgba(109,188,109,0.1)", transition: "all 0.2s", cursor: "pointer", fontFamily: "inherit" }}
              whileTap={{ scale: 0.97 }}>
              {copied ? <Check size={13} /> : <Copy size={13} />}
              {copied ? "Copied" : "Copy caption"}
            </motion.button>
            <motion.button onClick={handleDownload} className="flex-1 flex items-center justify-center gap-2"
              style={{ padding: "13px 8px", borderRadius: 12, fontSize: 12, fontWeight: 500, background: "rgba(13,31,13,0.5)", color: "#d4c9b0", border: "1px solid rgba(109,188,109,0.1)", cursor: "pointer", fontFamily: "inherit" }}
              whileTap={{ scale: 0.97 }}>
              <Download size={13} />
              Download
            </motion.button>
