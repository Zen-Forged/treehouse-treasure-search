...
// Mall profile page — all vendor finds from a specific peddler/antique mall.
// Buyers browse what's available before making the trip.
// URL: /mall/[slug]  e.g. /mall/rustic-roots-antiques
          ...
          {hasImg ? (
            <div style={{ position: "relative", width: "100%", paddingBottom: "75%" }}>
              ...
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "0 9px 8px" }}>
                ...
                {post.vendor && (
                  <div style={{ fontSize: 9, color: "#3a2e18" }}>{post.vendor.display_name}</div>
                )}
              </div>
            </div>
          ) : (
...
export default function MallPage() {
  ...
  useEffect(() => {
    if (!slug) return;
    Promise.all([getMallBySlug(slug), getMallBySlug(slug)])
      .then(async () => {
        const m = await getMallBySlug(slug);
          ...
          >
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
              <div>
                ...
                {mall.address && (
                  <div style={{ fontSize: 11, color: "#4a3a1e", paddingLeft: 14 }}>{mall.address}</div>
                )}
              </div>
